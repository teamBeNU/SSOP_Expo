...
import { eyesItems, eyebrowsItems, mouthItems, moleItems, hairFrontItems, hairBackItems, clothesItems, accEarItems, accNoseItems, accGlassesItems, accPinItems, accEtcItems, bgItems, bgColors } from "../../components/Avatar/avatarItems";
import { avatarCapture } from "../../utils/avatarCapture";

const screenWidth = Dimensions.get('window').width; // 화면의 전체 너비

export default function EditCardCover({navigation}) {
    const route = useRoute();
    const { editAvatar, cardId, cardCover } = route.params;

    const [avatar, setAvatar] = useState({});
    const [resetAvatar, setResetAvatar] = useState({});       // 초기화를 위해 초기값 보관할 아바타
    ...
    const [isReset, setIsReset] = useState(false);            // reset 여부
    ...
    const handleSubmit = async () => {
        ...
        try {
            const response = await axios.patch(`http://43.202.52.64:8080/api/card/edit?cardId=${cardId}`,
            formData, 
            {
                headers: {
        ...
        navigation.navigate('카드 상세보기', { cardId : cardId }); 
    };


    // 처음 렌더링 되었을 때 기본값(기존값)으로 지정
    useEffect(() => {
        if (cardCover === 'picture') {    // cardCover가 picture인 경우, 아바타 초기값 지정
            setAvatar({
                eyes: 1,
                eyebrows: 1,
                mouth: 1,
            ...
            setAccEtcSave(null);
            setBgSave(null);
        } else {                    // cardCover가 avatar인 경우, 기존값을 아바타 초기값으로 지정
            setAvatar({
                eyes: editAvatar.eyes,
                eyebrows: editAvatar.eyebrows,
                mouth: editAvatar.mouth,
                mole: editAvatar.mole,
                hairFront: editAvatar.hairFront,
                hairBack: editAvatar.hairBack,
                hairFrontColor: editAvatar.hairFrontColor,
                hairBackColor: editAvatar.hairBackColor,
                clothes: editAvatar.clothes,
                accEar: editAvatar.accEar,
                accNose: editAvatar.accNose,
                accGlasses: editAvatar.accGlasses,
                accPin: editAvatar.accPin,
                accEtc: editAvatar.accEtc,
                bg: editAvatar.bg,
                bgColor: editAvatar.bgColor,
            });
            setResetAvatar({
                eyes: editAvatar.eyes,
                eyebrows: editAvatar.eyebrows,
            ...
            setEyesSave(editAvatar.eyes);
            setEyebrowsSave(editAvatar.eyebrows);
            setMouthSave(editAvatar.mouth);
            setMoleSave(editAvatar.mole);
            setHairFrontSave(editAvatar.hairFront);
            sethHairBackSave(editAvatar.hairBack);
            setClothesSave(editAvatar.clothes);
            setAccEarSave(editAvatar.accEar);
            setAccNoseSave(editAvatar.accNose);
            setAccGlassesSave(editAvatar.accGlasses);
            setAccPinSave(editAvatar.accPin);
            setAccEtcSave(editAvatar.accEtc);
            setBgSave(editAvatar.bg);
        }
        setIsUndo(true);
    ...
    useEffect(() => {
        if (isReset) {
            ...
            if (JSON.stringify(undo.current[undo.current.length - 1]) === JSON.stringify(undo.current[undo.current.length - 2])) {
                ...
                setEyesSave(resetAvatar.eyes);
                setEyebrowsSave(resetAvatar.eyebrows);
                setMouthSave(resetAvatar.mouth);
                setMoleSave(resetAvatar.mole);
                setClothesSave(resetAvatar.clothes);
                setHairFrontSave(resetAvatar.hairFrontColor);
                sethHairBackSave(resetAvatar.hairBackColor);
                setAccEarSave(resetAvatar.accEar);
                setAccNoseSave(resetAvatar.accNose);
                setAccGlassesSave(resetAvatar.accGlasses);
                setAccPinSave(resetAvatar.accPin);
                setAccEtcSave(resetAvatar.bg);
                setBgSave(resetAvatar.bgColor);
                
                setIsMoleCancel(resetAvatar.mole ? false : true);
                setIsAccEarCancel(resetAvatar.accEar ? false : true);
                setIsAccNoseCancel(resetAvatar.accNose ? false : true);
                setIsAccGlassesCancel(resetAvatar.accGlasses ? false : true);
                setIsAccPinCancel(resetAvatar.accPin ? false : true);
                setIsAccEtcCancel(resetAvatar.accEtc ? false : true);
                setIsBgCancel(resetAvatar.bg ? false : true);
                setIsFaceSelect(true);
                setIsMoleSelect(true);
                setIsClothesSelect(true);
                setIsHairBackSelect(true);
